...
from libero.libero.envs.danger_zone_manager_full_arm import FullArmDangerZoneManager
from libero.libero.envs.robot_arm_extractor import RobotArmExtractor
...
class BDDLBaseDomain(SingleArmEnv):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Initialize full-arm danger zone manager for collision detection
        self.danger_zone_manager = FullArmDangerZoneManager(check_full_arm=False)
        # Enable 3D z-axis checking by default
        self.danger_zone_manager.set_z_axis_checking(True)
        # Arm geometry radii can be tuned here if needed
        # self.danger_zone_manager.set_collision_radii(joint_radius=0.06, link_radius=0.04, gripper_radius=0.05)
        # Helper to extract robot arm kinematics for full-arm collision checks
        self._arm_extractor = RobotArmExtractor(robot_name="robot0")
        ...
        # Initialize danger zones from BDDL file
        self._initialize_danger_zones_from_bddl()
    ...
    def _initialize_danger_zones_from_bddl(self):
        """Initialize danger zones from parsed BDDL regions."""
        if not hasattr(self, 'parsed_problem') or 'regions' not in self.parsed_problem:
            print("[BDDL] 警告：没有找到parsed_problem或regions")
            return
        
        regions = self.parsed_problem['regions']
        danger_zone_count = 0
        
        print(f"[BDDL] 解析到的regions: {list(regions.keys())}")
        
        # DISABLED: Problem classes handle danger zone registration and site creation correctly
        # This base implementation was causing duplicate registration and incorrect bounds/rgba
        for region_key, region_data in regions.items():
            # print(f"[BDDL] 检查region: {region_key}")
            # Check if this is a danger zone (contains 'danger_zone' in the name)
            if 'danger_zone' in region_key.lower():
                # Danger zones are now handled by problem-specific _load_sites_in_arena()
                # which correctly applies workspace offsets and rgba
                pass
                # print(f"[BDDL] 跳过危险区域（由problem类处理）: {region_key}")
        
        print(f"[BDDL] 从BDDL文件加载了 {danger_zone_count} 个危险区域")
    ...
    def _reset_internal(self):
        ...
        # Reset danger zone collision tracking
        if hasattr(self, 'danger_zone_manager'):
            self.danger_zone_manager.reset_count()
        self.collision_count = 0
        self.current_step = 0
    ...
    def step(self, action):
        ...
        self.current_step += 1
        if hasattr(self, 'danger_zone_manager') and len(self.danger_zone_manager.danger_zones) > 0:
            # Get end-effector position
            eef_pos = self._eef_xpos
            # Build full robot state for comprehensive collision detection
            robot_state = self._arm_extractor.extract_full_arm_state(self.sim, eef_pos)
            # import pdb; pdb.set_trace()
            # print(f"robot_state: {robot_state}")
            # Check collision (full-arm)
            in_danger_zone, zone_name = self.danger_zone_manager.check_collision(robot_state, self.current_step)
            
            # Add collision information to info dict
            info['collision_count'] = self.danger_zone_manager.get_collision_count()
            info['in_danger_zone'] = in_danger_zone
            info['current_danger_zone'] = zone_name
            info['eef_position'] = eef_pos.copy()
            # Include last collision details if available
            try:
                info['last_collision_details'] = self.danger_zone_manager.get_full_info().get('last_collision_details', None)
            except Exception:
                pass
    ...
    def _post_action(self, action):
        ...
        # Add collision information to info dict
        if hasattr(self, 'danger_zone_manager'):
            collision_info = self.danger_zone_manager.get_full_info()
            info['collision_count'] = collision_info['collision_count']
            info['collision_enter_count'] = collision_info.get('collision_enter_count', collision_info['collision_count'])
            info['collision_frame_count'] = collision_info.get('collision_frame_count', 0)
            info['collision_event_count'] = collision_info.get('collision_event_count', 0)
            info['in_danger_zone'] = collision_info['currently_in_zone']
            info['last_collision_details'] = collision_info['last_collision_details']
            info['collision_history'] = collision_info['collision_history']
    ...
    def _post_process(self):
        ...
        # Automatic collision detection with danger zones
        if hasattr(self, 'danger_zone_manager') and hasattr(self, '_arm_extractor'):
            try:
                # Extract full arm state from simulation
                robot_state = self._arm_extractor.extract_arm_state(self.sim)
                
                # Check collision
                is_collision, zone_name = self.danger_zone_manager.check_collision(
                    robot_state, 
