...
class Libero_Kitchen_Tabletop_Manipulation(BDDLBaseDomain):
    ...
    def _load_sites_in_arena(self, mujoco_arena):
        ...
        for object_region_name in list(region_dict.keys()):
            ...
            # Check if this region has numeric ranges (for both kitchen_table regions and danger zones)
            if "ranges" in region_dict[object_region_name] and len(region_dict[object_region_name]["ranges"]) > 0:
                ...
                if "danger_zone" in object_region_name:
                    ...
                    margin_xy = 0.02
                    table_height = self.kitchen_table_offset[2] if hasattr(self, "kitchen_table_offset") else 0.90
                    zone_bounds = {
                        'x_min': ranges[0] + self.kitchen_table_offset[0] - margin_xy,
                        'y_min': ranges[1] + self.kitchen_table_offset[1] - margin_xy,
                        'x_max': ranges[2] + self.kitchen_table_offset[0] + margin_xy,
                        'y_max': ranges[3] + self.kitchen_table_offset[1] + margin_xy,
                    ...
                    table_body_z = self.kitchen_table_offset[2] - self.kitchen_table_full_size[2] / 2
                ...
                elif "kitchen_table" in object_region_name:
                    # Use original rgba for kitchen_table regions
                ...
                # For danger zones, use pos directly; for kitchen_table regions, apply offset
                site_pos = target_zone.pos
                if "danger_zone" not in object_region_name:
                    site_pos = target_zone.pos + np.array([0.0, 0.0, -0.90])
