...
class Libero_Floor_Manipulation(BDDLBaseDomain):
    ...
    def _load_sites_in_arena(self, mujoco_arena):
        ...
        for object_region_name in list(region_dict.keys()):
            ...
            # Check if this region has numeric ranges (for floor regions and danger zones)
            if "ranges" in region_dict[object_region_name] and len(region_dict[object_region_name]["ranges"]) > 0:
                ...
                if "danger_zone" in object_region_name:
                    ...
                    zone_bounds = {
                        'x_min': ranges[0] - margin_xy,
                        'y_min': ranges[1] - margin_xy,
                        'x_max': ranges[2] + margin_xy,
                        'y_max': ranges[3] + margin_xy,
                        'z_min': floor_height,
                        'z_max': floor_height + 0.30,  # 30cm height for floor-level danger zones
                    }
                    ...
                    # Create TargetZone with visible height for floor-level visualization
                    target_zone = TargetZone(
                        name=object_region_name,
                        rgba=rgba,
                        zone_size=zone_size,
                        zone_centroid_xy=zone_centroid_xy,
                        zone_height=0.15,  # Half of 0.30 height
                        z_offset=0.15,  # Center at 0.15m above floor
                    )
                elif "floor" in object_region_name:
                    # Use original rgba for floor regions
