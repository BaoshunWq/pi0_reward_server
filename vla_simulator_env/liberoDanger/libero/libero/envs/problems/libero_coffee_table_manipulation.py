...
class Libero_Coffee_Table_Manipulation(BDDLBaseDomain):
    ...
    def _load_sites_in_arena(self, mujoco_arena):
        ...
        for object_region_name in list(region_dict.keys()):
            ...
            # Check if this region has numeric ranges (for coffee_table regions and danger zones)
            if "ranges" in region_dict[object_region_name] and len(region_dict[object_region_name]["ranges"]) > 0:
                ...
                if "danger_zone" in object_region_name:
                    # Use red semi-transparent color for danger zones (increased alpha for visibility)
                    rgba = [1.0, 0.0, 0.0, 0.1]
                    
                    # Add to danger zone manager with workspace offset applied
                    margin_xy = 0.02
                    table_height = self.coffee_table_offset[2] if hasattr(self, "coffee_table_offset") else 0.40
                    zone_bounds = {
                        'x_min': ranges[0] + self.coffee_table_offset[0] - margin_xy,
                        'y_min': ranges[1] + self.coffee_table_offset[1] - margin_xy,
                        'x_max': ranges[2] + self.coffee_table_offset[0] + margin_xy,
                        'y_max': ranges[3] + self.coffee_table_offset[1] + margin_xy,
                    ...
                    table_body_z = self.coffee_table_offset[2] - self.coffee_table_full_size[2] / 2
                ...
                elif "coffee_table" in object_region_name:
                    # Use original rgba for coffee_table regions
