...
class Args:
    ...
    env_name: str = "GoogleRobotOpenDrawer-v0"
...
def eval_one_task(args: Args) -> float:
    ...
    control_mode = get_robot_control_mode(args.robot, "rt1")
    kwargs = dict(
        obs_mode="rgbd",
        robot=args.robot,
    ...
    session_id = f"rt1-{args.env_name}-{args.scene_name}-{args.init_state_id}-{args.seed}"
    client.reset(session_id, args.instruction, policy_setup=args.policy_setup, action_scale=args.action_scale, saved_model_path=args.saved_model_path)
    total_successes = 0
    total_episodes = 0
