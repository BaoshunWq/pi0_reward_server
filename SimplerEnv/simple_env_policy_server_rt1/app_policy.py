...
def create_app():
    ...
    def reset():
        ...
        saved_model_path = body.get("saved_model_path") or "rt_1_x_tf_trained_for_002272480_step"
        model = sessions.get(session_id)
        if model is None:
            from simpler_env.policies.rt1.rt1_model import RT1Inference
            model = RT1Inference(saved_model_path=saved_model_path, policy_setup=policy_setup, action_scale=action_scale)
    ...
    def infer():
        ...
        return jsonify({"raw_action": {
            ...
            "gripper_closedness_action": np.asarray(raw_action["gripper_closedness_action"]).tolist(),
            "terminate_episode": np.asarray(raw_action["terminate_episode"]).tolist(),
        }, "action": {
            "world_vector": np.asarray(action["world_vector"]).tolist(),
            "rot_axangle": np.asarray(action["rot_axangle"]).tolist(),
