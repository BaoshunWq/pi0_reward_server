...
class OpenVLAOFTEvalConfig:
    ...
    task_stats_path: str = (
        f"./output/{local_model_path}/{NOW_TIME_STR}/openvla_oft_redteaming_results_task_stats.json"
    )
    output_path: str = (
        f"./output/{local_model_path}/{NOW_TIME_STR}/openvla_oft_redteaming_results.json"
    )
    examples_path: str = ""
...
def _evaluate_annotation(
    ...
    client = _websocket_client_policy.WebsocketClientPolicy(cfg.policy_host, cfg.policy_port)
    action_plan = collections.deque()
    total_episodes, total_successes = 0, 0
    video_saved = False

    try:
        for episode_idx in tqdm.tqdm(range(cfg.num_trials_per_annotation), desc="Episodes per annotation"):
            env.reset()
            ...
            while timesteps < max_steps + cfg.num_steps_wait:
                ...
                img = image_tools.convert_to_uint8(image_tools.resize_with_pad(img, cfg.resize_size, cfg.resize_size))
                ...
                if not action_plan:
                    ...
                    if len(action_chunk) < cfg.replan_steps:
                        raise RuntimeError(
                            f"Policy returned {len(action_chunk)} steps, but replan_steps={cfg.replan_steps}."
                        )
            ...
            if cfg.video_out_path and replay_images and not video_saved:
...
def _init_instruction_generator(cfg: OpenVLAOFTEvalConfig) -> InstructionGeneratorFacade:
...
@draccus.wrap()
def run_openvla_oft_ert(cfg: OpenVLAOFTEvalConfig):
    ...
    for task_id, task_language in tqdm.tqdm(list(enumerate(task_language_list)), desc="Tasks"):
        ...
        task_mean_similarity = (task_sim_sum / task_sim_count) if task_sim_count > 0 else None
        task_stats.append(
            {
                "task_suite_name": cfg.task_suite_name,
                "task_id": int(task_id),
    ...
    suite_mean_similarity = (suite_sim_sum / suite_sim_count) if suite_sim_count > 0 else None
    task_stats.append(
        {
            "task_suite_name": cfg.task_suite_name,
    ...
    logging.info(
        f"Overall success rate on {suite_task_num} episodes: {overall:.3f}, similarity of all generate: {suite_mean_similarity}"
    )
