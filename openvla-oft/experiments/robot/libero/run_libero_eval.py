...
class GenerateConfig:
    ...
    #################################################################################################################
    # Policy serving parameters
    #################################################################################################################
    policy_mode: PolicyExecutionMode = PolicyExecutionMode.LOCAL
...
def validate_config(cfg: GenerateConfig) -> None:
    ...
    if cfg.policy_mode == PolicyExecutionMode.LOCAL:
        assert cfg.pretrained_checkpoint, "pretrained_checkpoint must not be None for local policy execution!"
...
def connect_policy_client(cfg: GenerateConfig):
    """Initialize a websocket policy client when running in remote mode."""
    if cfg.policy_mode != PolicyExecutionMode.REMOTE:
        return None, {}

    logger.info(
        "Connecting to remote policy server at %s:%s",
...
def eval_libero(cfg: GenerateConfig) -> float:
    ...
    # Initialize model and components or connect to remote policy server
    model = action_head = proprio_projector = noisy_action_projector = processor = None
    policy_client = None
    if cfg.policy_mode == PolicyExecutionMode.LOCAL:
        model, action_head, proprio_projector, noisy_action_projector, processor = initialize_model(cfg)
    else:
        policy_client, _ = connect_policy_client(cfg)
