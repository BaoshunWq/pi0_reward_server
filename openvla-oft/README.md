...
## Remote Serving Workflow (OpenPI-style)

We now support the same policy/env separation flow used by `openpi`. This enables you to launch a long-running policy server and connect multiple evaluation workers (or the reward server in `pi0_reward_server`).
...
1. **Start the policy server**
   ...
   This loads the checkpoint once and exposes it over a websocket endpoint.

2. **Run evaluation workers (LIBERO simulator, reward server, etc.)**
   ...
   When `policy_mode` is set to `remote`, the evaluator skips local checkpoint loading and streams observations to the websocket server, mirroring the staged execution flow from `openpi/scripts`.
