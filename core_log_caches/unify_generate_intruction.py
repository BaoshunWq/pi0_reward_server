...
class EmbodiedRedTeamModelWithVERLQwen:
    ...
    def __init__(
        ...
        if lora_adapter_path:
            ...
            adapter_load_path = lora_adapter_path
            cleanup_root = None
            config_filenames = ("adapter_config.json", "config.json")
            try:
                from peft.config import PeftConfig
                from peft.mapping import PEFT_TYPE_TO_CONFIG_MAPPING
            except ImportError as exc:
                raise RuntimeError("[VERL-Qwen] Failed to import PEFT configuration utilities.") from exc
            allowed_base_keys = set(getattr(PeftConfig, "__dataclass_fields__", {}).keys())
            for filename in config_filenames:
                config_path = os.path.join(lora_adapter_path, filename)
                ...
                try:
                    ...
                    print(f"[VERL-Qwen] Warning: failed to read {config_path}: {exc}")
                    break
                allowed_keys = set(allowed_base_keys)
                peft_type = adapter_config.get("peft_type")
                config_cls = None
                if peft_type:
                    config_cls = PEFT_TYPE_TO_CONFIG_MAPPING.get(peft_type)
                    if config_cls is None:
                        print(f"[VERL-Qwen] Warning: unknown PEFT type '{peft_type}' in adapter config; proceeding with base fields only.")
                    else:
                        allowed_keys.update(getattr(config_cls, "__dataclass_fields__", {}).keys())
                unsupported_keys = sorted(key for key in adapter_config if key not in allowed_keys)
                if unsupported_keys:
                    if cleanup_root is None:
                        cleanup_root = tempfile.mkdtemp(prefix="verl_lora_adapter_")
                        target_dir = os.path.join(cleanup_root, "adapter")
                        shutil.copytree(lora_adapter_path, target_dir)
                    cleaned_config_path = os.path.join(target_dir, filename)
                    for key in unsupported_keys:
                        print(f"[VERL-Qwen] Detected unsupported '{key}' in adapter config; removing it for PEFT compatibility.")
                        adapter_config.pop(key, None)
                    with open(cleaned_config_path, "w", encoding="utf-8") as f:
                        json.dump(adapter_config, f, ensure_ascii=True, indent=2)
                    adapter_load_path = target_dir
                break
            try:
                self.model = PeftModel.from_pretrained(base_model, adapter_load_path)
...
def build_red_team_generator(
    ...
    **kwargs,
    ...
    elif backend == "verl_qwen" or backend == "qwen_llm":
